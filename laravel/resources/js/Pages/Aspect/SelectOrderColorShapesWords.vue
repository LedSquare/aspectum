...
<template>
    ...
    <div class="aspect-frame">
        ...
        <div style="display: flex; flex-direction: row; gap: 1rem">
            <div class="word-colors-box">
                <div :style="['background-color: ' + word.colorCode]" class="color-box"
                    v-for="(word, index) in leftWords" :key="word.id" draggable="true"
                    @dragstart="onStart($event, index)" @drop="onDrop($event, index)" @dragenter.prevent
                    @dragover.prevent>

                </div>
            </div>
            <div class="shape-colors-box">
                <div class="color-box" :style="['background-color: ' + word.shapeColorCode]" v-for="word in rigthWords">
                </div>
            </div>
        </div>
